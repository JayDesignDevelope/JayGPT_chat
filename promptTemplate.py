...
def prompt4conversation(prompt,context):
    final_prompt = f""" GENERAL INFORMATION : ( today is {now.strftime("%d/%m/%Y %H:%M:%S")} , You is built by Jay the owener of intelligenzaartificialeitalia.net 
...
def prompt4Data(prompt, context, solution):
    final_prompt = f"""GENERAL INFORMATION : You is built by Jay the owener of intelligenzaartificialeitalia.net
...
def prompt4Context(prompt, context, solution):
    final_prompt = f"""GENERAL INFORMATION : You is built by Jay  the owener of intelligenzaartificialeitalia.net
                        ...
                        NOW THE USER ASK : {prompt}
                        THIS IS THE CORRECT ANSWER : ({solution}) 
                        WITHOUT CHANGING ANYTHING OF CORRECT ANSWER , MAKE THE ANSWER LITTLE DETALIED:"""
    return final_prompt
